...
| 保留商品名称与尺寸 | eve35 使用 `SKU_1`、`SKU_2`，会丢失商品描述 | 保留完整文件名；按父目录或 finalized 商品名归组，多图保留“第一张/第二张+尺寸”；业务字段完全相同的重复行合并，不同订单/地址/关键词仍独立输出 |
...
Excel 打包只选择可交付素材；`.psd`、`.psb`、`.ai`、`.cdr`、`.eps`
等设计源文件不会进入结果包。仅有源文件而没有 JPG/PNG/PDF 等成品时，会在
缺失清单中标记“仅存在源文件，未找到成品素材”。
